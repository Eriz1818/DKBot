...
class DraganKnight:
        ...
        def get_rest(self):
            ...
            self.browser.select_form(nr=1)
            response = self.browser.submit(name='submit', label='Yes')
            self.update_stats(response.get_data())

        def run_from_fight(self):
            self.browser.select_form(nr=1)
            response = self.browser.submit(name='run', label='Run')
            self.update_stats(response.get_data())

        def do_fast_spell(self, spell_id):
            response = self.browser.open("http://dragon.se7enet.com/index.php?do=spell:%s" % spell_id)
            self.update_stats(response.get_data())

        def update_stats(self,html):

            #cleaning HTML (FOR MP)
            ...
            # print "Level: " + level_match.group(1)
            # print "Exp: " + exp_match.group(1).replace(",", "")
            # print "Gold: " + gold_match.group(1).replace(",", "")
            #
            # print "HP: " + hp_match.group(1)
            # print "MP: " + mp_match.group(1)
            # print "TP: " + tp_match.group(1)